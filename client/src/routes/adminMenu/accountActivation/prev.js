...
function AccountActivation() {
  ...
  useEffect(() => {
    ...
    const fetchRoles = async () => {
      try {
        const response = await axios.get(`${API_BASE_URL}5000/api/roles`)
    ...
    const fetchPositions = async () => {
      try {
        const response = await axios.get(`${API_BASE_URL}5000/api/positions`)
  ...
  const handleSave = (userId) => {
    ...
    axios
      .put(`${API_BASE_URL}5000/api/users/${userId}`, userWithoutRoleName, {
        headers: {
