...
const AgreedGroup = () => {
  ...
  return (
    <div>
      <MdContactSupport className="help-icon" onClick={handleOpenHelpModal} title="Справка" />
      <HelpModalWorkGroup
        type={'agreed'}
        open={openHelpModal}
      ...
      {groups.length === 0 ? (
        ...
        groups
          ...
          .map((group) => {
            const remainingTimeInfo = calculateRemainingTime(group.selected_date) // Вычисляем оставшееся время
            return (
              <div className="group-card" key={group.id}>
                {user.id === group.created_by && remainingTimeInfo.isExpired ? (
                  <button onClick={() => handleComplectGroup(group.id)} className="complete-button">
                    Завершить
                  </button>
                ) : user.id === group.created_by ? (
                  <button onClick={() => handleCancelGroup(group.id)} className="cancel-button">
                ...
                <strong style={importanceStyles[group.importance]}>{group.group_name}</strong>
                <p className="date">Дата: {formatDate(group.selected_date)}</p>

                <p className="remaining-time" style={{ color: remainingTimeInfo.color }}>
                  Осталось: <strong>{remainingTimeInfo.time}</strong>
                </p>

                <p className="weekday">День недели: {getWeekday(group.selected_date)}</p>

                <p className="description">Описание задачи: {group.description}</p>

                <h4 className="participants-header">Участники:</h4>
                <ul className="participant-list">
                  {Array.isArray(group.participants) && group.participants.length > 0 ? (
                    group.participants.map((participant, idx) => <li key={idx}>{participant}</li>)
                  ) : (
