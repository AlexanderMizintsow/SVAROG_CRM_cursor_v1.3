...
const MissedCalls = () => {
  ...
  const updateMissedCallWithReminder = useCallback(
    (callId, reminderId, reminderData) => {
      setMissedCalls((prevCalls) =>
        prevCalls.map((call) =>
          call.id === callId
            ? {
                ...call,
  ...
  const updateMissedCallWithTask = useCallback((callId, taskId, taskData) => {
    setMissedCalls((prevCalls) =>
      prevCalls.map((call) =>
        call.id === callId
          ? {
  ...
  const fetchUserSettings = useCallback(async () => {
    ...
    if (userId) {
      try {
        const response = await axios.get(
          `${apiBaseUrl}5004/api/calls-settings-missed/${userId}`
        );
  ...
  return (
    <div className="container">
      ...
      <NotificationList
        ...
        userInfo={userInfo}
        onReminderCreated={updateMissedCallWithReminder}
        onTaskCreated={updateMissedCallWithTask}
      />
