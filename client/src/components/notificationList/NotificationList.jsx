...
import { Schedule, Assignment } from "@mui/icons-material";
...
const NotificationList = ({
  ...
  const handleScheduleModalClose = (reminderId) => {
    ...
    if (reminderId) {
      // Обновляем звонок с ID созданного напоминания
      updateCallWithReminderId(activeNotificationId, reminderId);
    }
  };

  const updateCallWithReminderId = async (callId, reminderId) => {
    try {
      // Сначала получаем текущие данные звонка, чтобы сохранить существующий task_id
      const currentCallResponse = await axios.get(
        `${API_BASE_URL}5004/api/calls/${callId}/details`
      );
      const currentCallData = currentCallResponse.data;

      // Отправляем обновление, сохраняя существующий task_id
      const response = await axios.put(
        `${API_BASE_URL}5004/api/calls/${callId}`,
        {
          purpose_id: currentCallData.purpose_id,
          description: currentCallData.description,
          outcome: currentCallData.outcome,
          reminder_id: reminderId,
          task_id: currentCallData.task_id, // Сохраняем существующий task_id
        }
      );
      ...
      // Получаем данные о созданном напоминании
      const reminderResponse = await axios.get(
        `${API_BASE_URL}5004/api/reminders/${reminderId}`
      );
      const reminderData = reminderResponse.data;

      // Обновляем данные в родительском компоненте
      if (onReminderCreated) {
        onReminderCreated(callId, reminderId, {
          title: reminderData.title,
          comment: reminderData.comment,
          creatorName: reminderData.creator_name || "Неизвестно",
        });
      }
    } catch (error) {
      console.error("❌ Ошибка при обновлении звонка с ID напоминания:", error);
      Toastify({
        text: "Ошибка при связывании напоминания со звонком",
        close: true,
        style: {
  ...
  // Функция для перевода статусов задач на русский язык
  ...
  const fetchEmployees = async (dealerId) => {
    ...
    try {
      const response = await axios.get(
        `${API_BASE_URL}5004/api/dealer-employees`,
        {
  ...
  const fetchComments = async (callId) => {
    try {
      const response = await axios.get(
        `${API_BASE_URL}5004/api/call-comments/${callId}`
      );
  ...
  const fetchProcessedByUser = async (callId) => {
    try {
      const response = await axios.get(
        `${API_BASE_URL}5004/api/call-processor/${callId}`
      );
      return response.data;
    } catch (error) {
  ...
  const handleEditComment = async (callId) => {
    ...
    const existingComment =
      commentsData.length > 0 ? commentsData[0].comment : "";
    setComments((prev) => ({
      ...prev,
  ...
  const handleConfirmStatusChange = async () => {
    ...
    try {
      const response = await axios.patch(
        `${API_BASE_URL}5004/api/calls/${notification.id}`,
        {
          status: "processed",
        }
      );
  ...
  const handleBatchProcessCalls = async () => {
    ...
    try {
      const response = await axios.patch(
        `${API_BASE_URL}5004/api/calls/batch-process`,
        {
  ...
  const filterNotifications = (notifications) => {
    ...
    return notifications.filter((notification) => {
      ...
      return (
        String(notification.callerNumber)
          .toLowerCase()
          .includes(searchString) ||
        (notification.callerName &&
          notification.callerName.toLowerCase().includes(searchString)) ||
          ...
          {filterNotifications(notifications).map((notification) => (
            <div key={notification.id} className="notification-card">
              ...
              <p className="notification-receiver">
                Получатель: {notification.receiverNumber} (
                {notification.receiverName})
              </p>{" "}
              <span className="notification-time">
                ...
                employees[notification.dealerId].length > 0 ? (
                  <p className="notification-time">
                    Закрепленные сотрудники:{" "}
                    {employees[notification.dealerId]
                      .map((emp) => {
                        const firstInitial = emp.first_name
                          .charAt(0)
                          .toUpperCase();
                ...
                {notification.status === "processed" &&
                  processedByUsers[notification.id] && (
                    <div>
                      <p className="notification-time">
                        Обработал:{" "}
                        {(() => {
                          const firstInitial = processedByUsers[
                            notification.id
                          ].first_name
                            .charAt(0)
                            .toUpperCase();
                          const middleInitial = processedByUsers[
                            notification.id
                          ].middle_name
                            ...
                            : "";
                          const lastName =
                            processedByUsers[notification.id].last_name;
                      ...
                      <p className="notification-time">
                        ...
                        >
                          {new Date(
                            processedByUsers[notification.id].created_at
                ...
                {notification.reminderId && (
                  <Tooltip
                    title={
                      <div>
                        <div>
                          <strong>Напоминание создано</strong>
                        </div>
                        <div>ID: {notification.reminderId}</div>
                        <div>
                          Создатель:{" "}
                          {notification.reminderCreatorName || "Неизвестно"}
                        </div>
                        <div>
                          Заголовок:{" "}
                          {notification.reminderTitle || "Без заголовка"}
                        </div>
                        {notification.reminderComment && (
                          <div>
                            Комментарий:{" "}
                            {notification.reminderComment.substring(0, 100)}...
                          </div>
                        )}
                      </div>
                    }
                  ...
                  <Tooltip
                    title={
                      <div>
                        ...
                        <div>ID: {notification.taskId}</div>
                        <div>
                          Создатель:{" "}
                          {notification.taskCreatorName || "Неизвестно"}
                        </div>
                        <div>
                          Название: {notification.taskTitle || "Без названия"}
                        </div>
                        {notification.taskDescription && (
                          <div>
                            Описание:{" "}
                            {notification.taskDescription.substring(0, 100)}...
                          </div>
                        )}
                        {notification.taskExecutorsNames && (
                          <div>
                            Исполнители: {notification.taskExecutorsNames}
                          </div>
                        )}
                        {notification.taskStatus && (
                          <div>
                            Статус: {getTaskStatusText(notification.taskStatus)}
                          </div>
                        )}
                      </div>
                    }
