...
async function handle(instance, node, scheme, integrations, dbPool) {
  ...
  if (!userId) return { fail: 'Проект: не определён userId' }

  const text = substituteText(settings.text || '', context)
  if (!text) return { fail: 'Проект: текст сообщения пустой' }

  try {
    await reg.sendGlobalTaskChatMessage({
      globalTaskId: projectId,
      userId,
      text,
      title: settings.title || '',
      repliedToMessageId: settings.repliedToMessageId || null,
    })
  } catch (e) {
    return { fail: `Проект: не удалось отправить сообщение в чат: ${e?.message || 'ошибка'}` }
  }

  const edges = getOutgoingEdges(scheme, node.id)
  const nextEdge = edges[0]
  if (!nextEdge) return { fail: 'У узла «Проект: чат» нет исходящего ребра' }
